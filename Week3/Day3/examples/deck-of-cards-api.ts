...
export const drawCards = async (baseURL: string, deckID: string | null, number: number) => {
  const response = await axios.get(`${baseURL  }deck/${  deckID  }/draw/?count=${  number}`)
  return response['data']['cards']
}

export const drawCard = async (baseURL: string, deckID: string | null) => {
  const response = await drawCards(baseURL, deckID, 1)
  return response[0]
}

export const remainingCards = async (baseURL: string, deckID: string | null) => {
